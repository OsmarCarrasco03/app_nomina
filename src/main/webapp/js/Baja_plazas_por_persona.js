...
function obtenerInfoPersona(idPersona) {
	...
	Promise.all([promesaUrl1])
		.then(resultados => {
			...
			if (resultados[0].error) {
				swal({
					title: "La persona no tiene una plaza asignada",//""resultados[0].error,
					icon: 'error',
					button: 'Aceptar'
				});
...
function llenarTodosLosCampos(datosApiPlaza) {
	...
	idPlaza = datosApiPlaza.data[0][0];
    ...
    $('#btnDardeBajaPorPlaza').prop('disabled', false);
    ...
    $('#btnDardeBajaPorPersona').prop('disabled', false);
...
btnHabilitarBusqueda.addEventListener("click", function() {
	...
	$('#btnDardeBajaPorPlaza').prop('disabled', true);
	$('#btnDardeBajaPorPersona').prop('disabled', true);
...
// Inicio acción del boton BAJA DE BUSQUEDA POR PLAZA

btnDardeBajaPorPlaza.addEventListener("click", function() {
    console.log(idPlaza);
    console.log(idPersona);
    var fechaHoyTermino = new Date().toISOString().split('T')[0];
    console.log(fechaHoyTermino);
	const situacion = 2;

    

    swal({
		title: '¿Estás seguro de dar de baja esta plaza?',
		...
		if (willRegister) {
			...
			const url1 = `api/insertar/snplazapersonaha/${idPlaza}/0`;
			const url2 = `api/insertarfechatermino/snplazapersonaha/${idPlaza}/0/${fechaHoyTermino}`;
			const url2_5 = `api/actualizarestatusocupado/snplaza/${idPlaza}`;
			const url3 = `api/borrar/snplazapersona/${idPlaza}/0`;
			...
			// Para POST, se necesita especificar el método y, si es necesario, el cuerpo de la solicitud
			const promesaUrl2_5 = fetch(url2_5, {
			    method: 'POST',
			    headers: {
			        'Content-Type': 'application/json'
			    },
			...
			swal("Buen trabajo!", "Se dio de baja la plaza!", "success");
...
// Fin acción del boton BAJA DE BUSQUEDA POR PLAZA

// Inicio acción del boton BAJA DE BUSQUEDA POR PERSONA

btnDardeBajaPorPersona.addEventListener("click", function() {
    console.log(idPlaza);
    console.log(idPersona);
    var fechaHoyTermino = new Date().toISOString().split('T')[0];
    console.log(fechaHoyTermino);
    ...
    swal({
		title: '¿Estás seguro de dar de baja esta persona?',
		text: 'Una vez registrado, no podrás deshacer esta acción',
		icon: 'warning',
		buttons: ['Cancelar', 'Aceptar'],
		dangerMode: true,
		...
		if (willRegister) {
			...
			const url1 = `api/insertar/snplazapersonaha/0/${idPersona}`;
			const url2 = `api/insertarfechatermino/snplazapersonaha/0/${idPersona}/${fechaHoyTermino}`;
			const url2_5 = `api/actualizarestatusocupado/snplaza/${idPlaza}`;
			const url3 = `api/borrar/snplazapersona/0/${idPersona}`;
			...
			swal("Buen trabajo!", "Se dio de baja la persona!", "success");
...
// Fin acción del boton BAJA DE BUSQUEDA POR PERSONA
